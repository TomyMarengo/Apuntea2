...
ALTER TABLE Notes ADD COLUMN IF NOT EXISTS visible boolean DEFAULT true NOT NULL;
ALTER TABLE Notes ADD COLUMN IF NOT EXISTS file_type character varying(10) NULL;
ALTER TABLE Notes ADD COLUMN IF NOT EXISTS last_modified_at timestamp DEFAULT now() NOT NULL;

--update all null types to pdf for retro-compatibility
UPDATE Notes SET file_type = 'pdf' WHERE file_type IS NULL;

ALTER TABLE Notes ALTER COLUMN file_type SET NOT NULL;
...
CREATE VIEW InstitutionData AS (
...
ALTER TABLE Users ADD CONSTRAINT "FK_users_images" FOREIGN KEY (profile_picture_id) REFERENCES Images (image_id) ON DELETE SET NULL;
ALTER TABLE Users ADD COLUMN IF NOT EXISTS status character varying(10) DEFAULT 'ACTIVE' NOT NULL;
ALTER TABLE Users ADD CONSTRAINT "CK_users_status" CHECK (status IN ('ACTIVE', 'BANNED', 'DELETED'));
