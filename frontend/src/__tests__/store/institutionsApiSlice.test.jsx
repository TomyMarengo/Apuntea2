...
import {
    ...
    useGetCareerSubjectsByYearQuery, institutionsApiSlice,
...
import {
    someInstitutionId,
    someCareerId,
    someSubjectId, emptyNameMsg, invalidYearMsg, invalidName
...
describe("institutionsApiSlice", () => {
    ...
    it("should fetch a single institution by ID successfully", async () => {
        const { result } = useWithWrapper(() => useGetInstitutionQuery({ institutionId: someInstitutionId }), store);
        await waitFor(() => expect(result.current.isSuccess).toBe(true));
    ...
    it("should fetch careers by institution successfully", async () => {
        const { result } = useWithWrapper(() => useGetCareersQuery({ institutionId: someInstitutionId }), store);
        await waitFor(() => expect(result.current.isSuccess).toBe(true));
    ...
    it("should fetch a career by ID successfully", async () => {
        const { result } = useWithWrapper(() => useGetCareerQuery({ institutionId: someInstitutionId, careerId: someCareerId }), store);
        await waitFor(() => expect(result.current.isSuccess).toBe(true));
    ...
    it("should fetch a subject by ID successfully", async () => {
        ...
        const { result } = useWithWrapper(() => useGetSubjectQuery({ subjectId: someSubjectId }), store);
        await waitFor(() => expect(result.current.isSuccess).toBe(true));
    ...
    it("should create a subject successfully", async () => {
        const { result: wrapperResult } = await useWithWrapper(() => useCreateSubjectMutation(), store);
        const [createSubject] = await wrapperResult.current;
        const result = await createSubject({ name: "New Subject", year: 1, institutionId: someInstitutionId, careerId: someCareerId, }).unwrap();
    ...
    it("should fail to create subject due to empty name, should relay errors", async () => {
        const { result: wrapperResult } = await useWithWrapper(() => useCreateSubjectMutation(), store);
        const [createSubject] = await wrapperResult.current;
        const result = await createSubject({ name: "", year: 1, institutionId: someInstitutionId, careerId: someCareerId, }).unwrap();
    ...
    it("should fail to create subject due to invalid year, should relay errors", async () => {
        const { result: wrapperResult } = await useWithWrapper(() => useCreateSubjectMutation(), store);
        const [createSubject] = await wrapperResult.current;
        const result = await createSubject({ name: "New Subject", year: -5,  institutionId: someInstitutionId, careerId: someCareerId, }).unwrap();
        expect(result.success).toBe(false);
        expect(result.messages).contains(invalidYearMsg);
    });

    it("should modify a subject successfully", async () => {
        const { result: wrapperResult } = await useWithWrapper(() => useUpdateSubjectMutation(), store);
        const [updateSubject] = await wrapperResult.current;
        const result = await updateSubject({ name: "New Subject", year: 2025, institutionId: someInstitutionId, careerId: someCareerId, subjectId: someSubjectId}).unwrap();
        expect(result.success).toBe(true);
        expect(result.messages).empty;
    });
    ...
    it("should fail to modify subject due to empty name, should relay errors", async () => {
        const { result: wrapperResult } = await useWithWrapper(() => useUpdateSubjectMutation(), store);
        const [updateSubject] = await wrapperResult.current;
        const result = await updateSubject({ name: invalidName, year: 1, institutionId: someInstitutionId, careerId: someCareerId, subjectId: someSubjectId }).unwrap();
        expect(result.success).toBe(false);
        expect(result.messages).contains(emptyNameMsg);
    });
    ...
    it("should fail to modify subject due to invalid year, should relay errors", async () => {
        const { result: wrapperResult } = await useWithWrapper(() => useUpdateSubjectMutation(), store);
        const [updateSubject] = await wrapperResult.current;
        const result = await updateSubject({ name: "New Subject", year: -5,  institutionId: someInstitutionId, careerId: someCareerId, subjectId: someSubjectId }).unwrap();
        expect(result.success).toBe(false);
        expect(result.messages).contains(invalidYearMsg);
    });
    ...
    it("should get subjects not in career", async () => {
        const { result } = useWithWrapper(() => useGetSubjectsNotInCareerQuery({ careerId: someCareerId }), store);
        await waitFor(() => expect(result.current.isSuccess).toBe(true));
    ...
    it("should get subjects by career", async () => {
        const { result } = useWithWrapper(() => useGetSubjectsByCareerQuery({ careerId: someCareerId }), store);
        await waitFor(() => expect(result.current.isSuccess).toBe(true));
        const subjects = result.current.data;
        expect(subjects).not.empty;
    });
    ...
    it("should link subject with career successfully", async () => {
        const { result: wrapperResult } = await useWithWrapper(() => useLinkSubjectCareerMutation(), store);
        const [linkSubject] = await wrapperResult.current;
        const result = await linkSubject({ year: 1, institutionId: someInstitutionId, careerId: someCareerId, subjectId: someSubjectId}).unwrap();
        expect(result.success).toBe(true);
        expect(result.messages).empty;
    });
    ...
    it("should fail to link subject with career due to invalid year, should relay errors", async () => {
        const { result: wrapperResult } = await useWithWrapper(() => useLinkSubjectCareerMutation(), store);
        const [linkSubject] = await wrapperResult.current;
        const result = await linkSubject({ year: -5,  institutionId: someInstitutionId, careerId: someCareerId, subjectId: someSubjectId}).unwrap();
        expect(result.success).toBe(false);
        expect(result.messages).contains(invalidYearMsg);
    });

    it("should get subject's careers", async () => {
        const { result } = useWithWrapper(() => useGetSubjectCareersQuery({ institutionId: someInstitutionId, careerId: someCareerId }), store);
        await waitFor(() => expect(result.current.isSuccess).toBe(true));
    ...
    it("should get career's subjects by year", async () => {
        const { result } = useWithWrapper(() => useGetCareerSubjectsByYearQuery({ careerId: someCareerId, year: 1 }), store);
        await waitFor(() => expect(result.current.isSuccess).toBe(true));
        const subjects = result.current.data;
        expect(subjects).not.empty;
    });
