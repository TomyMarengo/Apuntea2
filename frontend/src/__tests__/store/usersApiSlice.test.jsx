...
import {
    useCreateUserMutation, useGetFollowersQuery, useGetFollowingsQuery,
    useGetLoggedUserQuery,
    useGetUserQuery, useGetUsersQuery, useIsFollowingUserQuery, useRequestPasswordChangeMutation,
    usersApiSlice, useUpdateUserMutation, useUpdateUserPasswordMutation, useUpdateUserStatusMutation
...
describe('usersApiSlice', () => {
    ...
    it('should fetch a user successfully', async () => {
        let userId = someUserId;
        const { result } = useWithWrapper(() => useGetLoggedUserQuery({ userId }), store);
        await waitFor(() => expect(result.current.isSuccess).toBe(true));
        const user = result.current.data;
        expect(user).toBeTruthy();
        expect(user.id).toBe(userId);
    ...
    it("should create a user successfully", async () => {
        let { result: wrapperResult} = await useWithWrapper(async ()=> await useCreateUserMutation(), store);
    ...
    it("should modify a user successfully", async () => {
        ...
        const result = await updateUser({userId: someUserId, username: nonExistingUsername}).unwrap();
    ...
    it("should update user status successfully", async () => {
        let { result: wrapperResult} = await useWithWrapper(async ()=> await useUpdateUserStatusMutation(), store);
        const [updateUserStatus] = await wrapperResult.current;
        const result = await updateUserStatus({userId: someUserId, status: 'ACTIVE'}).unwrap();
        expect(result.success).toBe(true);
        expect(result.messages).empty;
    });
    ...
    it("should fetch users without mail successfully", async () => {
        const { result } = useWithWrapper(() => useGetFollowersQuery({url: "/users?followedBy="}), store);
        await waitFor(() => expect(result.current.isSuccess).toBe(true));
        const {users, totalCount, totalPages } = result.current.data;
        expectToBePagedContent(users, totalCount, totalPages);
        for (let user of users) {
    ...
    it("should fetch users without mail successfully", async () => {
        const { result } = useWithWrapper(() => useGetFollowingsQuery({url: "/users?following="}), store);
        await waitFor(() => expect(result.current.isSuccess).toBe(true));
        const {users, totalCount, totalPages } = result.current.data;
        expectToBePagedContent(users, totalCount, totalPages);
        for (let user of users) {
    ...
    it("should get the user as followed", async () => {
        const { result } = useWithWrapper(() => useIsFollowingUserQuery({ userId: someUserId, followerId: followerUserId }), store);
        await waitFor(() => expect(result.current.isSuccess).toBe(true));
    ...
    it("should get the user as not followed", async () => {
        const { result } = useWithWrapper(() => useIsFollowingUserQuery({ userId: followerUserId, followerId: someUserId }), store);
        await waitFor(() => expect(result.current.isSuccess).toBe(true));
        const isFavorite = result.current.data.success;
    ...
    it("should request password change successfully", async () => {
        let { result: wrapperResult} = await useWithWrapper(async ()=> await useRequestPasswordChangeMutation(), store);
        const [requestPasswordChange] = await wrapperResult.current;
        const result = await requestPasswordChange({email: existingEmail}).unwrap();
    ...
    it("should fail request password change due to non existing mail", async () => {
        let { result: wrapperResult} = await useWithWrapper(async ()=> await useRequestPasswordChangeMutation(), store);
        const [requestPasswordChange] = await wrapperResult.current;
        const result = await requestPasswordChange({email: nonExistingEmail}).unwrap();
        expect(result).toBe(false);
    });

    it("should change the password successfully", async () => {
        let { result: wrapperResult} = await useWithWrapper(async ()=> await useUpdateUserPasswordMutation(), store);
        const [requestPasswordChange] = await wrapperResult.current;
        const result = await requestPasswordChange({ userId: someUserId, password: "1234", email: existingEmail, code: passwordChangeCode }).unwrap();
        expect(result.success).toBe(true);
    });
    ...
    it("should change the password successfully", async () => {
        let { result: wrapperResult} = await useWithWrapper(async ()=> await useUpdateUserPasswordMutation(), store);
        const [requestPasswordChange] = await wrapperResult.current;
        const result = await requestPasswordChange({ userId: someUserId, password: "1234", email: existingEmail, code: "" }).unwrap();
        expect(result.success).toBe(false);
    });
});
