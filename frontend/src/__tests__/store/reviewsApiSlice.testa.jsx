...
describe('reviewsApiSlice', () => {
    ...
    it('should fetch a single review successfully', async () => {
        const { result } = useWithWrapper(() => useGetReviewQuery({ noteId: someNoteId, userId: someUserId }), store);
        await waitFor(() => expect(result.current.isSuccess).toBe(true));
    ...
    it('should fetch my review successfully', async () => {
        const { result } = useWithWrapper(() => useGetMyReviewQuery({ url: `/reviews?noteId=${someNoteId}`, userId: someUserId }), store);
        await waitFor(() => expect(result.current.isSuccess).toBe(true));
        const review = result.current.data;
        expect(review).toBeTruthy();
        expect(review.userId).toBe(someUserId);
    ...
    it('should create a review successfully', async () => {
        let { result: wrapperResult } = await useWithWrapper(async () => await useCreateReviewMutation(), store);
        const [createReview] = await wrapperResult.current;
        const result = await createReview({ noteId: someNoteId, score: 5 }).unwrap();
    ...
    it('should fail to create a review with invalid score and relay error messages', async () => {
        let { result: wrapperResult } = await useWithWrapper(async () => await useCreateReviewMutation(), store);
        const [createReview] = await wrapperResult.current;
        const result = await createReview({ noteId: someNoteId, score: -1 }).unwrap();
        expect(result.success).toBe(false);
        expect(result.messages).contain(invalidScoreMsg);
    });

    it('should modify a review successfully', async () => {
        let { result: wrapperResult } = await useWithWrapper(async () => await useUpdateReviewMutation(), store);
        const [updateReview] = await wrapperResult.current;
        const result = await updateReview({ noteId: someNoteId, score: 5 }).unwrap();
        expect(result.success).toBe(true);
        expect(result.messages).empty;
    });
    ...
    it('should fail to modify a review with invalid score and relay error messages', async () => {
        let { result: wrapperResult } = await useWithWrapper(async () => await useUpdateReviewMutation(), store);
        const [updateReview] = await wrapperResult.current;
        const result = await updateReview({ noteId: someNoteId, score: -1 }).unwrap();
        expect(result.success).toBe(false);
        expect(result.messages).contain(invalidScoreMsg);
    });
