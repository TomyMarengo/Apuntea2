...
import {
    directoriesApiSlice,
    useCreateDirectoryMutation, useGetIsFavoriteDirectoryQuery,
    useGetDirectoryQuery, useGetUserDirectoriesFavoritesQuery, useUpdateDirectoryMutation
...
describe('directoriesApiSlice', () => {
    it('should fetch a directory successfully', async () => {
        ...
        const { result } = useWithWrapper(() => useGetDirectoryQuery({ directoryId }), store);
    ...
    it("should create a directory successfully", async () => {
        ...
        const result = await createDirectory({name: nonExistingDirectoryName}).unwrap();
    ...
    it("should modify directory successfully with nonExistingDirectoryName", async () => {
        ...
        const result = await updateDirectory({directoryId: existingDirectoryId, name: nonExistingDirectoryName}).unwrap();
        expect(result.success).toBe(true);
        expect(result.messages).empty;
    });
    ...
    it("should modify directory successfully with no name change", async () => {
        let { result: wrapperResult} = await useWithWrapper(async ()=> await useUpdateDirectoryMutation(), store);
        const [updateDirectory] = await wrapperResult.current;
        const result = await updateDirectory({directoryId: existingDirectoryId, visible: false}).unwrap();
        expect(result.success).toBe(true);
        expect(result.messages).empty;
    });
    ...
    it("should modify directory successfully with the same old name", async () => {
        let { result: wrapperResult} = await useWithWrapper(async ()=> await useUpdateDirectoryMutation(), store);
        const [updateDirectory] = await wrapperResult.current;
        const result = await updateDirectory({directoryId: existingDirectoryId, name: existingDirectoryName}).unwrap();
        expect(result.success).toBe(true);
        expect(result.messages).empty;
    });
    ...
    it("should get the directory as favorite", async () => {
        const { result } = useWithWrapper(() => useGetIsFavoriteDirectoryQuery({ directoryId: existingDirectoryId, userId: favUserId }), store);
        await waitFor(() => expect(result.current.isSuccess).toBe(true));
    ...
    it("should get the directory as not favorite", async () => {
        const { result } = useWithWrapper(() => useGetIsFavoriteDirectoryQuery({ directoryId: nonFavDirectoryId, userId: favUserId }), store);
        await waitFor(() => expect(result.current.isSuccess).toBe(true));
        const isFavorite = result.current.data.success;
    ...
    it('should fetch favorite directories succesfully', async () => {
        let directoryId = existingDirectoryId;
        const { result } = useWithWrapper(() => useGetUserDirectoriesFavoritesQuery({}), store);
        await waitFor(() => expect(result.current.isSuccess).toBe(true));
